...
const getCodecKey = (): string => {
  const key = process.env.NEXT_PUBLIC_PLAYER_URL_CODEC_KEY?.trim() || "";
  console.log("[playerUrlCodec] getCodecKey() - env variable:", key ? `[HIDDEN - ${key.length} chars]` : "NOT SET");
  return key;
};
...
export const decodePlayerStreamUrl = (value: string): string => {
  ...
  console.log("[playerUrlCodec] decodePlayerStreamUrl called:", {
    hasKey: !!key,
    keyLength: key?.length || 0,
    inputValue: value?.substring(0, 80),
    isEncoded: value?.startsWith(URL_CODEC_PREFIX),
  });
  
  if (!key) {
    console.warn("[playerUrlCodec] No codec key available, returning value as-is");
    return value;
  }
  if (!value || !value.startsWith(URL_CODEC_PREFIX)) {
    console.log("[playerUrlCodec] Value not encoded (no enc: prefix), returning as-is");
    return value;
  }
  ...
  try {
    ...
    if (payload.length <= NONCE_LENGTH) {
      console.warn("[playerUrlCodec] Payload too short");
      return value;
    }
    ...
    const decoded = decoder.decode(plainBytes);
    console.log("[playerUrlCodec] Successfully decoded:", decoded.substring(0, 100));
    return decoded;
  } catch (e) {
    console.error("[playerUrlCodec] Decode error:", e);
    return value;
  }
};
