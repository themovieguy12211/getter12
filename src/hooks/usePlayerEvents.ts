...
export interface UsePlayerEventsOptions { media?: { id: string | number; type: ContentType }; metadata?: { season?: number; episode?: number }; saveHistory?: boolean; trackUiState?: boolean; adBlockDetected?: boolean; onPlay?: (data: UnifiedPlayerEventData) => void; onPause?: (data: UnifiedPlayerEventData) => void; onSeeked?: (data: UnifiedPlayerEventData) => void; onEnded?: (data: UnifiedPlayerEventData) => void; onTimeUpdate?: (data: UnifiedPlayerEventData) => void; }
export function usePlayerEvents(options: UsePlayerEventsOptions = {}) {
  const { data: user } = useSupabaseUser();
  const documentState = useDocumentVisibility();
  const { media, metadata, saveHistory, trackUiState = true, adBlockDetected, onPlay, onPause, onSeeked, onEnded, onTimeUpdate } = options;
  ...
  const normalizePayload = (data: UnifiedPlayerEventData): UnifiedPlayerEventData => ({ ...data, mediaId: media?.id ?? data.mediaId ?? 0, mediaType: media?.type ?? data.mediaType ?? "movie", season: metadata?.season ?? data.season ?? 0, episode: metadata?.episode ?? data.episode ?? 0, activeWatchSeconds: data.activeWatchSeconds ?? activeWatchSecondsRef.current, playerSource: data.playerSource ?? "external", adBlockDetected: adBlockDetected ?? data.adBlockDetected ?? false });
