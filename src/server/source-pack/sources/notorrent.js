    ...
    try {
        ...
        for (const item of apiData.streams) {
            if (!item.externalUrl && item.url && !item.url.includes("github.com") && !item.url.includes("googleusercontent")) {
                ...
                allUrls.push({ url: item.url, quality: extractQuality(item.title || ""), server: `NoTorrent - ${item.name || 'Server'}`, type: item.url.includes(".m3u8") ? "hls" : "mp4", headers: Object.keys(headers).length ? headers : { 'User-Agent': 'Mozilla/5.0' }, skipProxy: true });
            }
        }
