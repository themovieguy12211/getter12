...
async function getVidupMeta(id, s, e) {
    try {
        ...
        const decServersData = await fetchJson(`${API_BASE}/dec-vidup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: serversEncrypted }) });
...
export async function getStream({ id, s, e, server }) {
    ...
    const results = await Promise.allSettled(targets.map(async srv => {
        ...
        const decStreamData = await fetchJson(`${API_BASE}/dec-vidup`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: streamEncrypted }) });
        if (decStreamData.status !== 200 || !decStreamData.result?.url) throw new Error();
        return { url: decStreamData.result.url, server: `VidUp - ${srv.name || 'Server'}`, type: decStreamData.result.url.includes('.m3u8') ? 'hls' : 'mp4', subtitles: (decStreamData.result.captions || []).map(c => ({ url: c.file, lang: c.label || 'Unknown' })), headers: { ...HEADERS, 'Origin': DOMAIN }, skipProxy: true };
    }));
    const allUrls = [];
...
export async function getSources(args) {
    const meta = await getVidupMeta(args.id, args.s, args.e);
    return meta?.servers ? meta.servers.map(srv => `VidUp - ${srv.name}`) : [];
}
