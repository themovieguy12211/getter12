...
function sha256Into(buf, total, out) {
    ...
    for (let off=0; off<total; off+=64) {
        ...
        for (let i=16;i<64;i++){const a=W_BUF[i-15],b=W_BUF[i-2];const s0=((a>>>7)|(a<<25))^((a>>>18)|(a<<14))^(a>>>3);const s1=((b>>>17)|(b<<15))^((b>>>19)|(b<<13))^(b>>>10);W_BUF[i]=(W_BUF[i-16]+s0+W_BUF[i-7]+s1)|0;}
        let a=h0,b=h1,c=h2,d=h3,e=h4,f=h5,g=h6,h=h7;
        for(let i=0;i<64;i++){const S1=((e>>>6)|(e<<26))^((e>>>11)|(e<<21))^((e>>>25)|(e<<7));const ch=(e&f)^(~e&g);const t1=(h+S1+ch+K[i]+W_BUF[i])|0;const S0=((a>>>2)|(a<<30))^((a>>>13)|(a<<19))^((a>>>22)|(a<<10));const maj=(a&b)^(a&c)^(b&c);const t2=(S0+maj)|0;h=g;g=f;f=e;e=(d+t1)|0;d=c;c=b;b=a;a=(t1+t2)|0;}
...
export async function getStream(args) {
    ...
    try {
        ...
        if (server && server !== "all") { const clean = server.replace("Mapple - ", ""); targets = SERVER_LIST.filter(sv => sv.name === clean); if (!targets.length) targets = SERVER_LIST; }
        const allUrls = [];
        for (const srv of targets) {
            try {
                ...
                if (streamData.success && streamData.data?.stream_url) {
                    ...
                    allUrls.push({ url: fileUrl, server: `Mapple - ${srv.name}`, headers: PROXY_HEADERS, skipProxy: true });
                }
            } catch {}
        }
