...
export async function getStream({ id, s, e }) {
    try {
        ...
        if (isTv) { params.append('season', s); params.append('episode', e); }

        const res = await fetch(`${API_BASE}?${params}`, { headers: HEADERS, signal: AbortSignal.timeout(15000) });
        ...
        for (const line of lines) {
            ...
            try {
                ...
                if (json.t === 'done' && json.data?.url) { streamData = json.data; break; }
            } catch { continue; }
        }
...
export async function getSources() { return ['OpStream']; }
