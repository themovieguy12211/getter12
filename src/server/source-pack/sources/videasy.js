...
const HEADERS = { "Accept": "*/*", "Origin": "https://player.videasy.to",
"Referer": "https://player.videasy.to/", "User-Agent": USER_AGENT };
const SERVERS = [
    { id: 'cdn', name: 'Yoru' },
...
export async function getStream({ id, s, e, server, sdk }) {
    ...
    try {
        ...
        const seedData = await fetchJson(`${SPEEDRACE_BASE}/seed?mediaId=${id}`,
{ headers: HEADERS, signal: AbortSignal.timeout(5000) });
        ...
        const settled = await Promise.allSettled(targets.map(async srv => {
            let url = `${SPEEDRACE_BASE}/${srv.id}/sources-with-title?title=${encTitle}&mediaType=${isTv
? 'tv' : 'movie'}&year=${info.year || ''}&tmdbId=${id}&imdbId=${info.imdbId ||
"tt0000000"}&enc=2&seed=${seed}`;
